# MarketHub Website
## Date: 19/11/2023
## By: Hussain Jaffer Aliwi || Hasan Ali Salman
####  [GitHub](https://github.com/H-aliwi/MarketHub-Website) 

***
### ***_Description_***
#### **The online marketplace system aims to create a website that facilitates the buying and selling of goods, catering to both shop owners and customers. Users can set up virtual shops, list their products, and effectively promote them to a broad audience. Customers can browse through various categories, explore options, and purchase desired products. The project integrates a real-time messaging feature, enabling communication between users within the system.** 


***

### ***_Technologies Used_***
* PHP (Pure PHP)
* MySQL
* JavaScript  
* Ajax / jQuery (js libraries) 
* HTML
* CSS 
* DataTables
* Bootstrap 
***

### ***Application users***
* User
* Shop Owner
* Admin

***

### ***Entity Relationship Diagram (ERD)***
***


[<img src="/E-shop/public/images/ERD.png" width="850" height="700"/>]()

***

### ***Project Plan ***
***

[<img src="/E-shop/public/images/Project plan.png" width="650" height="700"/>]()


***

### ***Wireframes***

***



[<img src="/E-shop/public/images/home-customer.png" width="420" height="430"/>]()
[<img src="/E-shop/public/images/home-shop-owner.png" width="420" height="430"/>]()
***
[<img src="/E-shop/public/images/shop_register.png" width="420" height="430"/>]()
[<img src="/E-shop/public/images/product_page.png" width="420" height="430"/>]()
***
[<img src="/E-shop/public/images/chat_room.png" width="420" height="430"/>]()
[<img src="/E-shop/public/images/chat_messages.png" width="420" height="430"/>]()



***

### ***Main Screenshots***

***



[<img src="/E-shop/public/images/Customer home page .jpg" width="650" height="400"/>]()
[<img src="../MarketHub-Website/E-shop/public/images/Shop owner home page .jpg" width="650" height="400"/>]()
***
[<img src="/E-shop/public/images/Shop owner register page .jpg" width="650" height="400"/>]()
[<img src="/E-shop/public/images/Generate report page – Admin.jpg" width="650" height="400"/>]()

***
[<img src="/E-shop/public/images/Available chat rooms page.PNG" width="650" height="400"/>]()
[<img src="/E-shop/public/images/Chat room page.PNG" width="650" height="400"/>]()



***
[<img src="/E-shop/public/images/Shops list page .jpg" width="650" height="400"/>]()
[<img src="/E-shop/public/images/Cart page.jpg" width="650" height="400"/>]()







***



### ***_Future Updates_***
- [ ] Integrated Delivery System
- [ ] Develop a Mobile Application
  
***
### ***_Credits_***
##### Images: [GeeksforGeeks](https://pixabay.com/)
##### Markdown Guide: [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
##### DataTable: [DataTable](https://datatables.net/)
##### XAMPP: [XAMPP](https://www.apachefriends.org/)
##### phpmyadmin: [phpmyadmin](https://www.phpmyadmin.net/)
##### Draw.io: [Draw.io](https://app.diagrams.net/)

***
