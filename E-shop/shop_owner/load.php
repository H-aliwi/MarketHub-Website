...
<?php
session_start();
include('includes/functions/getCustomerUsernameByUserId.php');
...
// echo "Room =" .$Room_id ."And receicer name :".$receiver_name;
...
if ($stat->rowCount() > 0) {
    while ($row = $stat->fetch()) {
      ...
      $IS_shop_owner=CkeckSenderMsg($senderID,$user);
          // echo "Is shop: ".$IS_shop_owner;

      if ($IS_shop_owner==1 )
      {
        $ownerID=$_SESSION['shop_onwer_id'];
        $sender = getShopName($ownerID);
      }
      else{ 
        $userId=$row['sender_id'];
        $Customer_username=getCustomerUsernameByUserId($userId);
        
        $sender = $Customer_username ;   }
